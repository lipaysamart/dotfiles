/**
 * pi-skill-palette — Skill command palette for Pi
 *
 * Ported from https://github.com/nicobailon/pi-skill-palette (v1.2.0)
 * Merged with PR#4: ancestor directory scanning for .agents/skills and .pi/skills.
 *
 * Usage: /skill — Opens the skill picker overlay.
 * Select a skill and it's sent alongside your next message.
 *
 * Changes from upstream:
 *  - Adapted imports for @earendil-works/pi-coding-agent and @earendil-works/pi-tui
 *  - Merged PR#4 ancestor directory scanning (walks up to git root)
 *  - Theme file loaded from ~/.pi/agent/extensions/pi-skill-palette-theme.json
 *    (flat file since this is a single-file extension, not a directory)
 *  - Removed cwd/.pi/skills from hardcoded list (ancestor loop covers it)
 *  - Config file loaded from ~/.pi/agent/extensions/pi-skill-palette-config.json
 *    (disablePaths: skip paths by substring, ancestorScan: enable/disable ancestor scan)
 */
...
interface PaletteConfig {
	disablePaths: string[];  // Path substrings to skip (e.g. [".codex", ".claude"])
	ancestorScan: boolean;   // Enable/disable ancestor directory scanning entirely
}
...
function loadConfig(): PaletteConfig {
	const configPath = path.join(os.homedir(), ".pi", "agent", "extensions", "pi-skill-palette-config.json");
	try {
		if (fs.existsSync(configPath)) {
			const content = fs.readFileSync(configPath, "utf-8");
			const custom = JSON.parse(content) as Partial<PaletteConfig>;
...
const paletteConfig = loadConfig();
...
/**
 * Check if a directory path should be skipped based on config.
 */
function isPathDisabled(dir: string): boolean {
	return paletteConfig.disablePaths.some((p) => dir.includes(p));
}

/**
 * Load skills from known directories.
 *
 * Static directories (scanned first, first occurrence wins):
 ...
 * 5. ~/.pi/skills — recursive (legacy)
 * 6. ~/.agents/skills — recursive
 *
 * Ancestor scan (after static dirs, controlled by paletteConfig.ancestorScan):
 * 7. .agents/skills and .pi/skills in each ancestor directory,
 *    walking up from cwd to git root or filesystem root.
 *
 * Paths can be disabled via paletteConfig.disablePaths (substring match).
 */
function loadSkills(): Skill[] {
	...
	if (paletteConfig.ancestorScan) {
		...
		while (currentDir !== path.dirname(currentDir)) {
			...
			if (!isPathDisabled(agentsSkills)) scanSkillDir(agentsSkills, "recursive", skillsByName);
			if (!isPathDisabled(piSkills)) scanSkillDir(piSkills, "recursive", skillsByName);
