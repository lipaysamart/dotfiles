...
// ── 提示音 (与 pi-ringing.ts 一致的节流/播放逻辑) ──
...
const SOUND_DIR = path.join(homedir(), ".pi/sounds/warcraft3");
...
function playSound(filename: string): void {
  ...
  const proc = spawn("afplay", ["-v", "0.5", fullPath], {
    detached: true,
    stdio: "ignore",
  });
  proc.unref();
...
function playBlockingSound(): void {
  const sounds = ["PeonWhat2.wav", "PeonWhat3.wav"];
  playSound(sounds[Math.floor(Math.random() * sounds.length)]);
}

function playEditorSound(): void {
  playSound("PeonReady1.wav");
}

function playConfirmedSound(): void {
  const sounds = ["PeonWorkComplete.wav", "PeonYes1.wav"];
  playSound(sounds[Math.floor(Math.random() * sounds.length)]);
} 
